...
# context: fork      # Optional: fork, inject, none
# agent: my-agent    # Optional: subagent to run in when context is fork
# shell: bash        # Optional: preferred shell for scripts
---
...
### Step 2 — Determine operation and risk tier
...
Classify the operation before executing. Copy the three tier definitions verbatim
from `PROMPT_ENGINEERING.md` ("Risk-tiered approval gates") — they are maintained
there, once — and list this skill's own operations under each:

- **No approval**: <the read-only operations this skill offers>
- **Bounded approval**: <the scoped operational task this skill supports>
- **Fresh approval**: <the writes, deletes, exports, and out-of-scope actions>

### Step 3 — Build the action plan

For bounded-approval and fresh-approval operations, present a plan to the user.
...
1. **Use the risk tier before acting.** Read-only operations inside scope need no approval; scoped operational plans need bounded approval; high-risk actions need fresh approval.
2. **Require fresh approval for high-risk actions.** Writes, deletes, publishes, deploys, restarts, exec, permission changes, admin/security changes, external messages, bulk actions, sensitive exports, or leaving scope require a fresh plan and approval.
3. **Never print or log credentials.** Only confirm that environment variables are set.
4. **<Operation ordering rule, e.g., "Create in dependency order: parents before children.">**
5. The manifest file (`.<tool-name>-manifest.json`) is auto-maintained by scripts. Do not edit manually.
6. If an operation fails, stop and report the error — do not continue with dependent operations.
...
- Keep the SKILL.md body inside the enforced size limits in `SKILL_SPEC.md`
  (Progressive disclosure); move detailed schemas to `references/CONFIG.md`
