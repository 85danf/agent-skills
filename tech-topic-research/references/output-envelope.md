...
> **For spawned subagents:** if a tech-topic-research specialist file (`references/specialists/<role>.md`) sent you here, read § Shape and § Anti-fabrication. The Shape section is the JSON envelope you must produce verbatim; Anti-fabrication is the WebFetch-every-URL discipline and the `[unverified]` cap rule.
...
`references/specialists/<role>.md` instructions still apply); the envelope makes synthesis
