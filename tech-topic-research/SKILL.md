...
compatibility: >
  Requires a host that exposes web search and web page fetching, and a way to
  ask the user clarifying questions (e.g. Claude Code's AskUserQuestion, or a
  plain conversational prompt as fallback). Parallel research fan-out is
  optional: on a host with subagent/parallel-task dispatch (e.g. Claude Code's
  Task tool with native subagent registration and run_in_background), each
  specialist role in `references/specialists/` is dispatched as its own
  parallel subagent. On a host without subagent dispatch, the main agent
  performs each specialist's steps itself, sequentially. The skill is
  read-only: it issues web searches and reads web pages, never writes to
  remote services. No config file or credentials are required.
---
...
- On hosts that support it, spawn parallel subagents (e.g. via Claude Code's
  Task tool) that also use WebSearch and WebFetch; on hosts that don't, perform
  the same specialist steps sequentially in the main agent.
...
1. The host runtime exposes web search and web fetch. If a parallel
   subagent/Task-tool mechanism is unavailable, the skill still runs Standard
   and Deep tiers — the main agent performs each specialist's steps itself,
   sequentially, instead of fanning them out in parallel (see "Platform-agnostic
   fan-out" below).
...
2. The host advertises web search and web fetch. If neither is available, stop
   and report that deep research cannot run on this host.
3. Check whether the host supports parallel subagent/Task dispatch (e.g.
   Claude Code's Task tool with native subagent registration and
   `run_in_background`). If it does, Phase 3 dispatches each specialist as
   its own parallel subagent. If it does not, Phase 3 still runs Standard
   and Deep tiers — the main agent performs each specialist's steps itself,
   sequentially — see "Platform-agnostic fan-out" under Phase 3.

## Skill Directory

Supporting files:

- `references/specialists/<role>.md` — one file per research specialist
  (docs-searcher, community-searcher, tutorial-searcher, integration-searcher,
  comparison-searcher, deep-analyst, synthesis-reviewer). Each file is that
  specialist's full assignment: what to research, how to search, and the
  markdown + JSON output it must produce. On a host with subagent dispatch,
  use a role's file verbatim as that subagent's prompt. On a host without it,
  read the file yourself and follow it as your own instructions for that step.
...
2. **Get a current overview from the live web. Mandatory — never
   ...
   returns "tool not available", follow `references/specialists/docs-searcher.md`
   yourself in gist mode (`Mode: gist` → 5-bullet gist, footnote ¹) — or, on a
   host with subagent dispatch, spawn it as a subagent instead.
   If both fail, stop and tell the user the host has neither live-web
   nor a way to run the docs-searcher gist fallback. Do NOT fall back to
   training data.
...
2. Read `references/teaching-tone.md` and use it to shape the explanation
   style.
3. Fan out to the specialist roles for this run's tier (see "Platform-agnostic
   fan-out" below).

**Standard depth** uses four searcher specialists: `docs-searcher`, `community-searcher`, `tutorial-searcher`, `integration-searcher` (`references/specialists/<role>.md`).

**Deep depth** adds `deep-analyst` (always) and `comparison-searcher` (when alternatives are a focus area, or always in comparison mode).

**Comparison mode** doubles each searcher pool — run each role **twice**, once per item. The `comparison-searcher` is single-instance and receives both items. See [`references/comparison-mode.md`](references/comparison-mode.md) for equal-treatment rules.

### Platform-agnostic fan-out

This skill fans out research to several specialist roles, one file per role under `references/specialists/`. How the fan-out actually runs depends on what the host supports:

- **Host with subagent/parallel-task dispatch** (e.g. Claude Code's Task tool with native subagent registration and `run_in_background`): dispatch each specialist as its own subagent, in parallel, using the corresponding `references/specialists/<role>.md` file verbatim as that subagent's prompt/system instructions. Spawn the whole wave in a single message so they run concurrently.
- **Host without subagent dispatch**: perform each specialist's steps yourself, sequentially, in the same order they're listed above (searchers first, then `deep-analyst` for Deep tier), then perform the synthesis step (Phase 4 onward) exactly as you would after subagents return. Read each specialist file as your own instructions for that step, produce its markdown-summary-plus-JSON-envelope output, move to the next specialist, and only then synthesize.

Either way, the assignment given to each specialist is the same:
...
For `comparison-searcher`, the assignment carries both items plus a `comparison_mode: true` line instead of the fields above (see the file itself for its distinct input contract). Structured output envelope contract canonical in [`references/output-envelope.md`](references/output-envelope.md); each specialist step reads it to produce its envelope.

If dispatching subagents, draft Phase 4 from your own knowledge plus Phase 1 findings while they run. If running sequentially, proceed straight to Phase 4 once every specialist for this tier has produced its output.
...
**For the Deep tier additionally**, run the `synthesis-reviewer` specialist
([`references/specialists/synthesis-reviewer.md`](references/specialists/synthesis-reviewer.md))
as a final reviewer — dispatched as a subagent on hosts that support it,
or performed by the main agent as its own step otherwise. The reviewer reads
the drafted Deep document and emits issues at HIGH / MEDIUM / LOW severity.
...
2. **Quick depth uses zero specialist fan-out.** Do not run any specialist
   role for a Quick request.
3. **On hosts with subagent dispatch, spawn all specialists for a wave in a
   single message** with `run_in_background: true`, so they run in parallel.
   On hosts without subagent dispatch, run each specialist's steps yourself,
   sequentially, in the fixed order given in Phase 3.
4. **WebFetch (or equivalent) every URL before including it; drop URLs that 4xx/5xx without substitution.** Canonical contract in [`references/output-envelope.md` § Anti-fabrication](references/output-envelope.md).
...
8. **Platform-agnostic fan-out contract.** This skill's specialist roles live
   in `references/specialists/<role>.md`. On a host with subagent/parallel-task
   dispatch, dispatch each as its own subagent in parallel. On a host without
   it, perform each specialist's steps yourself, sequentially, in the same
   order, then perform the synthesis step — do not skip specialists just
   because parallel dispatch is unavailable.
